...
namespace MVCFlowerShopWeek3.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<MVCFlowerShopWeek3User> _userStore;
        private readonly IUserEmailStore<MVCFlowerShopWeek3User> _emailStore;
        private readonly ILogger<RegisterModel> _logger;
        private readonly IEmailSender _emailSender;

        public RegisterModel(
            UserManager<MVCFlowerShopWeek3User> userManager,
            IUserStore<MVCFlowerShopWeek3User> userStore,
        ...
        public async Task OnGetAsync(string returnUrl = null)  // submit commonly use this
        {
            ReturnUrl = returnUrl;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            if (ModelState.IsValid) //if input form is valid
            {
                var user = CreateUser();
        ...
        private MVCFlowerShopWeek3User CreateUser()
        {
            try
            {
                return Activator.CreateInstance<MVCFlowerShopWeek3User>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(MVCFlowerShopWeek3User)}'. " +
                    $"Ensure that '{nameof(MVCFlowerShopWeek3User)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<MVCFlowerShopWeek3User> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<MVCFlowerShopWeek3User>)_userStore;
        }
    }
}
